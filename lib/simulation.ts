 ...
 *   - ノードを搔んでいる間だけ alphaTarget を 0.3 に上げ、
 *     搔んだノードは fx/fy でポインタに固定、まわりがバネで追従する。
 *   - 離すと alphaTarget を 0 に戻し、余韵を残してゆっくり沈靜化する。
...
const DISTANCE: Record<RelationType, number> = {
  member: 48, follow: 1500, overlap: 1100, next: 34, pre: 60, alt: 84, counter: 98,
}
/** バネの強さ。別視点は弱くして「うっすら引き合う」程度に */
const STRENGTH: Record<RelationType, number> = {
  member: 0.5, follow: 0.03, overlap: 0.008, next: 0.45, pre: 0.32, alt: 0.06, counter: 0.1,
}
...
export class Simulation {
  ...
  constructor(graph: Graph, inherit?: Simulation) {
    ...
    this.sim = forceSimulation<SimNode>()
      .stop() // 内蔵タイマーは使わない
      .alphaMin(ALPHA_MIN)
      .alphaDecay(0.02)      // 沈靜化の速さ。小さいほど長く漂う
