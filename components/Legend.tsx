...
export default function Legend() {
  return (
    <div className="pointer-events-none absolute bottom-2.5 left-2.5 rounded-[10px] border border-line bg-panel/90 px-2.5 py-2 text-[10px] leading-[1.85] text-muted backdrop-blur max-[640px]:text-[9.5px]">
      ...
      {RELATIONS.map((t) => {
        ...
        return (
          <span key={t} className="block">
            <i
              className="mr-1.5 inline-block h-0 w-[14px] border-t-2 align-middle"
      ...
      {STARS.map(([s, c]) => (
        <span key={s} style={{ color: c }} className="mr-1">
      ...
      <span className="block">
        ...
        <span style={{ color: '#fbbf24' }}>●</span> 未読 — 領域の色
      </span>
    </div>
  )
}
