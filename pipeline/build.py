...
def disp_title(t):
    ...
    if len(parts) > 1:
        head, rest = parts[0], parts[1:]
        keep = [p for p in rest if VOL.match(p.translate(TRANS))]
        if len(head) >= 4 or keep:
            return (head + (' ' + ' '.join(keep) if keep else '')).strip()
        return ' '.join(parts[:2])
    return t
...
EXTRA = [
 ...
 ("エピクロスの処方箱","夏川草介",2025,"本屋大賞2026ノミネート","lit"),
 ("正義論","ジョン・ロールズ",1971,"哲学・思想の古典","phil"),
 ("これからの「正義」の話をしよう","マイケル・サンデル",2010,"哲学・思想の定番","phil"),
 ("戦争は女の顔をしていない","スヴェトラーナ・アレクシェーヴィチ",1985,"世界文学の古典的必読書","lit"),
 ("ロボットとわたしの不思議な旅","ベッキー・チェンバーズ",2021,"SFが読みたい!2026海外篇","sf"),
 ("ありか","瀬尾まい子",2025,"本屋大賞2026ノミネート","lit"),
 ("失われた貌","桜田智也",2025,"このミス2026国内1位","mys"),
 ("ブレイクショットの軌跡","逢坂冗馬",2025,"このミス2026国内6位","lit"),
 ("マーブル館殺人事件","アンソニー・ホロヴィッツ",2024,"このミス2026海外 2位","mys"),
 ("夜明けまでに誰かが","ホリー・ジャクソン",2025,"このミス2026海外 5位","mys"),
 ("世界の終わりの最後の殺人","スチュアート・タートン",2024,"このミス2026海外 9位","sf"),
...
# --- (b) 同じ賞・同じ年 = 同時代の別視点 --------------------------------
AWARD_YEAR = re.compile(r'(本屋大賞|直木賞|若色也屋作節|このミス|本格ミステリ大賞|日本推理作家協会賞|日本SF大賞|星雲賞|ヒューゴー賞|ネビュラ賞|SFが読みたい!|日本ホラー小説大賞|講談社科学出版賞|ビジネス書大賞|ビジネス書グランプリ|じんぶん大賞|新書大賞)[^0-9]*(\d{4})')
yc = defaultdict(list)
for n in NODES:
    for s in n['src']:
...
SEED = [
 ("種の起源","利己的な遗伝子","pre","進化論の原典 → 遗伝子視点への転回"),
 ("利己的な遗伝子","延長された表現型","pre","同著者による理論の拡張"),
 ("利己的な遗伝子","サピエンス全史 上","pre","進化的な人間観が土台になる"),
 ...
 ("十角館の殺人","屏人荘の殺人","pre","クローズドサークル再発明の系謱"),
 ...
 ("デューン 砂の惑星","ハイペリオン","alt","巨大SF叙事詩の系謱"),
 ...
 ("沈黙の春","利己的な遗伝子","alt","1960-70年代、生命観を変えた二冊"),
 ("夜と霜","エピクロスの処方箱","alt","極限と日常、「良く生きる」への二つの接近"),
 ...
 ("統計学が最強の学問である","ファスト&スロー","alt","データで殴るか、認知の癒を見るか"),
 ...
 ("近畿地方のある場所について","残穑","pre","ドキュメント形式ホラーの先行作"),
 ...
 ("ソラリス","三体","pre","理解できない知性を書く系謱の原点"),
 ("利己的な遗伝子","日本人の9割が知らない遗伝の真実","pre","遗伝子の視点を先に入れておく"),
...
# --- (e) 孤立ノード救済：同じ領域の年代が近い本に繋ぐ --------------------
def degmap():
    d = defaultdict(int)
    for (s,t,_) in E: d[s] += 1; d[t] += 1
    return d
deg = degmap()
...
for n in NODES:
    ...
    for j in (i-1, i+1, i-2, i+2):
        if 0 <= j < len(pool) and pool[j]['k'] != n['k']:
            if link(n, pool[j], 'alt', '同じ領域の、近い時代の本'):
                stat['rescue'] += 1
                deg[n['k']] += 1
        if deg[n['k']] >= 2: break
